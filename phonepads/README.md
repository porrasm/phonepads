...
```bash
dotnet build phonepads/Phonepads.slnx
```

```bash
dotnet test phonepads/Phonepads.slnx
```

## Running

```bash
dotnet run --project phonepads/src/Phonepads.App
```
...
```bash
dotnet publish phonepads/src/Phonepads.App -c Release -p:PublishTrimmed=true -p:DebugType=none -o publish
```
...
|---|---|
| `../shared/Phonepads.Protocol` | Wire types, source-generated JSON, the HTTP setup/create calls and the WebSocket session — shared with mobile-kbm |
